...
export default function OrderDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      getPublicProfileById(orderData.jastiper_id)
        .then(setJastiperProfile)
        .catch(() => {});
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 space-y-6">
        ...
        {loading ? (
          ...
          <>
            ...
            {jastiperProfile ? (
              <Link
                href={`/jastiper/${jastiperProfile.username}`}
                className="rounded-2xl bg-white p-6 shadow-sm block hover:shadow-md transition"
              >
                <h2 className="text-sm font-semibold text-gray-900 mb-3">Jastiper</h2>
                <div className="flex items-center gap-3">
                  {jastiperProfile.profile_picture_url ? (
                    ...
                    <div className="h-10 w-10 rounded-full bg-(--color-primary)/10 flex items-center justify-center text-(--color-primary) font-semibold text-sm">
                      {(jastiperProfile.full_name || jastiperProfile.username || '?')[0].toUpperCase()}
                    </div>
                  )}
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900">
                      {jastiperProfile.full_name || jastiperProfile.username}
                    </p>
