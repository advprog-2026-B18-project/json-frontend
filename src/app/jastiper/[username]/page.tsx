...
function ProductCard({ product }: { product: ProductResponse }) {
  return (
      <Link
          ...
          className="group rounded-xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition overflow-hidden block"
      >
        ...
        <div className="p-3">
          ...
          <p className="mt-1 text-sm font-semibold text-primary-dark">{formatRupiah(product.price)}</p>
...
export default function JastiperProfilePage() {
  ...
  // Fetch products jastiper catalog catalog
  const fetchProducts = useCallback(async (q: string, pg: number) => {
    if (!username) return;
    setProductsLoading(true);
    try {
      // FIX CRITICAL: pg - 1 dikonversi karena penampung Pageable data Spring Boot bernilai 0-based index
  ...
  // Render State: Not Found Profile Guard
  ...
  return (
      <div className="min-h-screen bg-gray-50">
        ...
        {/* Profile Jastiper Info Board */}
        <div className="bg-white border-b border-gray-100">
          <div className="mx-auto max-w-4xl px-4 py-8">
            {profile.status === 'PENDING_VERIFICATION' && (
                <div role="alert" className="mb-4 rounded-lg border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm text-yellow-800">
                  Akun ini sedang dalam proses review verifikasi kartu identitas (KYC) oleh admin.
                </div>
            )}
            {profile.status === 'BANNED' && (
                <div role="alert" className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
                  Perhatian: Akun jastiper ini telah ditangguhkan sementara oleh platform.
                </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
              ...
              <div className="shrink-0">
                {profile.profile_picture_url ? (
                    ...
                    <div className="h-24 w-24 rounded-full bg-primary flex items-center justify-center text-white text-3xl font-bold shadow-xs">
                      {profile.username[0].toUpperCase()}
                    </div>
                )}
              </div>

              {/* Profile Meta Info Deck */}
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{profile.username}</h1>
                  <StatusBadge status={profile.status} />
                </div>
                {profile.full_name && (
                    <p className="text-gray-600 mb-1 font-medium">{profile.full_name}</p>
                )}
                <p className="text-xs text-gray-400 mb-3">
                  Bergabung sejak {formatDate(profile.member_since)}
                </p>

                {/* Jastiper Stats Module */}
                {profile.stats && (
                    <div className="flex flex-wrap gap-6 mb-3 border-t border-gray-50 pt-3">
                      <div className="text-left">
                        <p className="text-lg font-bold text-gray-800 tracking-tight">{profile.stats.total_orders}</p>
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-400">Reputasi Penjual</p>
                      </div>
                    </div>
                )}
        ...
        <div className="mx-auto max-w-4xl px-4 py-8">
          <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <h2 className="text-lg font-bold text-gray-800">
              Katalog Produk Jastip {profile.username}
            </h2>
            {/* Search Bar Input */}
            <div className="relative w-full sm:w-64">
              <input
                  ...
                  className="w-full rounded-lg border border-gray-300 py-2 pl-9 pr-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary bg-white"
              />
          ...
          {productsLoading ? (
              ...
              <div className="py-16 text-center rounded-xl bg-white border border-gray-100 p-6">
                <p className="text-sm text-gray-500">
                  {search ? 'Tidak ada produk jastip yang cocok dengan kata kunci pencarian Anda.' : 'Jastiper ini belum merilis atau menambahkan produk aktif di katalog.'}
                </p>
                {search && (
                    <button
                ...
                {pagination.total_pages > 1 && (
                    <div className="mt-8 flex items-center justify-center gap-2">
                      <button
                          ...
                          disabled={page <= 1}
                          className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer transition"
                      >
                        ← Sebelumnya
                      </button>
