...
function Toast({
  ...
  return (
      ...
      >
        <span>{message}</span>
        <button onClick={onClose} aria-label="Tutup notifikasi" className="ml-1 opacity-80 hover:opacity-100">
          ✕
        </button>
      </div>
  );
        ...
        <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
          ...
          <div className="mb-5">
            ...
            <textarea
                ...
                className="w-full resize-none rounded-xl border border-gray-300 px-3 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary"
            />
          ...
          <div className="flex gap-3">
            ...
            <button
                ...
                disabled={!canSubmit}
                className="flex-1 rounded-xl bg-primary py-2.5 text-sm font-semibold text-white hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
              {loading ? (
...
export default function AdminCatalogPage() {
  ...
  }, [accessToken, search, statusFilter, jastiperIdFilter, page]);
  ...
  async function handleModerate(action: ModerationAction, reason: string) {
    ...
    setModerating(true);
    try {
      const updated = await adminModerateProduct(accessToken, currentId, action, reason);
      ...
      const actionLabel = MODERATION_OPTIONS.find((o) => o.value === action)?.label ?? action;
      setToast({
  ...
  const selectCls =
      'rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary';
        ...
        <div className="mx-auto max-w-7xl px-4 py-8">
          ...
          <div className="mb-6 flex items-center justify-between">
            ...
            <button
                ...
                className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary-dark transition shadow-sm cursor-pointer"
            >
              + Kategori
            </button>
          </div>
          ...
          <div className="mb-5 flex flex-wrap gap-3">
            ...
            <div className="relative flex-1 min-w-52">
              <input
                  ...
                  className="w-full rounded-lg border border-gray-300 py-2 pl-9 pr-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary"
              />
            ...
            <input
                ...
                className="w-64 rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700 placeholder:text-gray-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary"
            />
          </div>

          {/* Fetch error */}
          {fetchError && (
              <div role="alert" className="mb-4 flex items-center justify-between rounded-xl bg-red-50 px-4 py-3 text-sm text-red-600">
                <span>{fetchError}</span>
                <button onClick={fetchProducts} className="ml-2 underline">Coba lagi</button>
              </div>
          )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Produk</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Jastiper</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Harga</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Stok</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Status</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Tgl. Beli</th>
                    ...
                    products.map((product) => {
                      ...
                      return (
                          <tr key={currentId} className="transition hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              {jastiperUsername ? (
                                  <Link href={`/jastiper/${jastiperUsername}`} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-700 transition hover:text-primary">
                                    {jastiperUsername}
                                  </Link>
                              ) : (
                            ...
                            <td className="px-4 py-3 text-sm font-medium text-gray-800">{formatRupiah(p.price)}</td>
                            <td className="px-4 py-3"><span className={`text-sm font-medium ${p.stock === 0 ? 'text-red-500' : 'text-gray-700'}`}>{p.stock}</span></td>
                            <td className="px-4 py-3"><StatusBadge status={p.status} /></td>
                            <td className="px-4 py-3 text-xs text-gray-500">{purchaseDate ? formatDate(purchaseDate) : '-'}</td>
          ...
          {!loading && totalPages > 1 && (
              <div className="mt-5 flex items-center justify-between">
                <p className="text-sm text-gray-500">Halaman {displayPage} dari {totalPages} &mdash; {totalItems} produk</p>
                <div className="flex items-center gap-1">
                  <button onClick={() => setPage(0)} disabled={page <= 0} className="rounded-lg border border-gray-300 px-2.5 py-1.5 text-xs text-gray-700 hover:bg-gray-50 disabled:opacity-40 cursor-pointer">«</button>
                  <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page <= 0} className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-40 cursor-pointer">← Sebelumnya</button>

                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    const start = Math.max(0, Math.min(page - 2, totalPages - 5));
                    return start + i;
                  }).map((p) => (
                      <button key={p} onClick={() => setPage(p)} className={`rounded-lg border px-3 py-1.5 text-sm cursor-pointer ${p === page ? 'border-primary bg-primary text-white' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>{p + 1}</button>
                  ))}

                  <button onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-40 cursor-pointer">Berikutnya →</button>
                  <button onClick={() => setPage(totalPages - 1)} disabled={page >= totalPages - 1} className="rounded-lg border border-gray-300 px-2.5 py-1.5 text-xs text-gray-700 hover:bg-gray-50 disabled:opacity-40 cursor-pointer">»</button>
                </div>
              </div>
          )}
        </div>

        {moderateTarget && <ModerationModal product={moderateTarget} onConfirm={handleModerate} onClose={() => setModerateTarget(null)} loading={moderating} />}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" role="dialog" aria-modal="true">
              <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                ...
                <form onSubmit={handleCreateCategory}>
                  <div className="mb-5">
                    <label htmlFor="cat-name" className="mb-1.5 block text-sm font-medium text-gray-700">Nama Kategori <span className="text-red-500">*</span></label>
                    <input id="cat-name" type="text" required value={categoryName} onChange={(e) => setCategoryName(e.target.value)} placeholder="Contoh: Elektronik, Pakaian..." className="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm text-gray-900 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary" />
                  </div>
                  <div className="flex gap-3">
                    <button type="button" onClick={() => { setIsCategoryOpen(false); setCategoryName(''); }} disabled={categoryCreating} className="flex-1 rounded-xl border border-gray-300 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition cursor-pointer">Batal</button>
                    <button type="submit" disabled={categoryCreating || !categoryName.trim()} className="flex-1 rounded-xl bg-primary py-2.5 text-sm font-semibold text-white hover:bg-primary-dark transition cursor-pointer">
