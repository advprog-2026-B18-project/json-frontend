...
import { Navbar } from '@/components/Navbar'; // Menggunakan komponen Navbar global cerdas posisi tengah
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="mx-auto max-w-5xl px-4 py-8">
          <main className="space-y-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Antrian Verifikasi KYC</h1>
              {!loading && (
                  <p className="mt-1 text-sm text-gray-500">Menampilkan {totalItems} pengajuan dari pengguna</p>
              )}
            </div>

            {/* Status filter tabs */}
            <div className="flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Filter status KYC">
              {STATUS_TABS.map((tab) => (
                  <button
                      key={tab.value}
                      role="tab"
            ...
            {loading ? (
                ...
                <div className="rounded-xl bg-white p-6 text-center shadow-sm border border-gray-100">
                  ...
                  <button
                      onClick={fetchData}
                      className="rounded-lg bg-primary px-4 py-2 text-sm text-white hover:bg-primary-dark transition cursor-pointer"
                  >
                    Coba lagi
                  </button>
                </div>
                ...
                <EmptyState
                    ...
                    description={statusFilter !== 'ALL' ? 'Tidak ada pengajuan dengan status seleksi ini' : 'Belum ada pengajuan KYC yang perlu ditinjau oleh administrator'}
                />
            ) : (
                <>
                  ...
                  <div className="overflow-x-auto rounded-xl bg-white shadow-sm border border-gray-100">
                    <table className="w-full text-sm">
                      ...
                      {items.map((item) => (
                          <tr key={item.kyc_id} className="hover:bg-gray-50/70 transition">
                            ...
                            <td className="px-4 py-3 text-right">
                              <button
                                  ...
                                  className="rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-white hover:bg-primary-dark transition cursor-pointer"
                              >
                                Review
                              </button>
                              {item.status === 'PENDING_VERIFICATION' && (
                                  <button
                                      ...
                                      className="ml-2 rounded-lg border border-green-300 px-3 py-1.5 text-xs font-medium text-green-700 hover:bg-green-50 transition disabled:opacity-50 cursor-pointer"
                                  >
                                    Setujui
                                  </button>
                              )}
        ...
        {reviewItem && (
            ...
            >
              <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="space-y-3 text-sm border-b border-gray-100 pb-4">
                  ...
                  <div>
                    ...
                    <span className="ml-2 text-gray-600">{formatDate(reviewItem.submitted_at)}</span>
                  </div>
                  <div>
                ...
                {reviewItem.status === 'PENDING_VERIFICATION' && (
                    <div className="mt-6">
                      {/* Action select tabs toggle */}
                      <div className="flex gap-3 mb-4">
                        <button
                            onClick={() => { setReviewAction('APPROVE'); setRejectionReason(''); }}
                            className={`flex-1 rounded-xl border-2 p-3 text-sm font-medium transition cursor-pointer ${
                                reviewAction === 'APPROVE'
                      ...
                      {/* Rejection input area — only when REJECT selected */}
                      {reviewAction === 'REJECT' && (
                          <div className="mb-4">
                            <label htmlFor="rejection-reason" className="mb-1.5 block text-sm font-medium text-gray-700">
                              Alasan Penolakan Dokumen <span className="text-red-500">*</span>
                            </label>
                            <textarea
                                id="rejection-reason"
                                value={rejectionReason}
                                onChange={(e) => setRejectionReason(e.target.value)}
                                placeholder="Contoh: Foto KTP buram, data tidak sesuai profile..."
                                rows={3}
                                disabled={reviewSubmitting}
                                className="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary disabled:bg-gray-50 resize-none"
                            />
                          </div>
                      )}

                      {/* Submit Action Confirmation Button */}
                      {reviewAction && (
                          <button
                              onClick={handleReviewConfirm}
                              disabled={reviewSubmitting || (reviewAction === 'REJECT' && !rejectionReason.trim())}
                              className={`w-full flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer ${
                                  reviewAction === 'APPROVE'
                          ...
                          >
                            ...
                            {reviewSubmitting
                                ? 'Memproses...'
                                : reviewAction === 'APPROVE'
                                    ? 'Konfirmasi Setujui & Ubah Role Akun'
                ...
                {reviewItem.status !== 'PENDING_VERIFICATION' && (
                    <p className="mt-4 text-sm text-gray-400 italic text-center">
                      Pengajuan verifikasi KYC ini sudah selesai diproses.
                    </p>
                )}
              </div>
            </div>
